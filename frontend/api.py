...
# =========================================================
# [New] 파싱된 텍스트를 백엔드 Ingest(분석/저장)로 전송 추가됨.
# =========================================================
def ingest_file_to_backend(text: str, upload_type: str) -> Tuple[bool, str]:
    """
    프론트엔드에서 파싱한 텍스트를 백엔드로 보내서
    캐릭터/세계관을 자동 추출 및 저장하도록 요청합니다.

    :param text: 파일에서 추출한 텍스트 (parse_file_content 결과)
    :param upload_type: 'character' 또는 'world'
    """
    if not text or not text.strip():
        return False, "텍스트 내용이 비어있습니다."

    # 백엔드 라우터 경로 (/story/ingest)
    url = f"{BASE_URL}/story/ingest"

    headers = {"Content-Type": "application/json; charset=utf-8"}
    payload = {
        "text": text,
        "type": upload_type  # 'character' | 'world'
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if res_json.get("status") == "success":
                # 성공 메시지 반환 (예: "캐릭터 저장 완료 (3명): 김태평, ...")
                return True, res_json.get("message", "처리 완료")
            else:
                return False, res_json.get("message", "처리 실패")
        else:
