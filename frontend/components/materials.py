...
def render_materials():
    ...
    with c_list:
        ...
        for mat in proj['materials']:
            ...
            if st.button(f"{icon} {mat['title']}", key=f"m_{mat['id']}", use_container_width=True, type="primary" if is_sel else "secondary"):
    ...
    with c_edit:
        ...
        if sel_mat:
            with st.container(border=True):
                ...
                new_c = st.selectbox("분류", ["역사", "설정", "인물", "기타"], index=["역사", "설정", "인물", "기타"].index(sel_mat['category']))
                ...
                if st.button("💾 저장하기", type="primary", use_container_width=True):
                    if save_material_api(sel_mat): st.toast("저장 완료!", icon="✅")
                    else: st.error("저장 실패")
        else:
