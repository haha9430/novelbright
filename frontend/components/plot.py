...
# [중요] api.py에서 함수들을 가져옵니다.
from api import parse_file_content, _save_world_and_plot_json, save_world_setting_api
...
# [중요] def _save_world_and_plot_json(...) 삭제됨! (api.py 것을 사용)
...
def render_plot(proj=None):  # 유연성을 위해 인자 추가
    ...
    # [수정됨] 파일로 세계관 내용 추가하기
    # --------------------------------
    with st.expander("📂 파일 내용 추가하기 (세계관 설정)"):
        st.caption("TXT, PDF, Word, HWP 파일을 업로드하면 텍스트를 추출하여 이어 붙입니다.")

        # [수정됨] type 리스트에 확장자 추가
        uploaded_world = st.file_uploader(
            "파일 업로드",
            type=["txt", "md", "pdf", "docx", "hwp"],  # 확장자 추가
            key="world_file_uploader"
        )

        if uploaded_world and st.button("내용 추가 적용"):
            # api.py의 parse_file_content가 텍스트를 깔끔하게 뽑아줍니다.
            new_text = parse_file_content(uploaded_world)

            if new_text:
                old_desc = world.get("desc", "")
                combined_desc = f"{old_desc}\n\n--- [추가 자료: {uploaded_world.name}] ---\n{new_text}"

                world["desc"] = combined_desc
                _save_world_and_plot_json(combined_desc)

                st.toast(f"'{uploaded_world.name}' 내용이 추가되었습니다!", icon="✅")
                st.rerun()
