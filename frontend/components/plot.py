...
def render_plot():
    ...
    # ---------------------------------------------------------
    # ✅ [NEW 추가] 세계관 파일 업로드 분석 (Ingest 연결)
    # ---------------------------------------------------------
            ...
            if st.button("🚀 AI 분석 및 저장 시작", key="world_ingest_btn", use_container_width=True):
                with st.spinner("AI가 세계관을 분석 중입니다..."):
                    ...
                    if text and not text.startswith("[Error]"):
                        # api.py의 ingest 함수 호출
                        if ingest_file_to_backend(text, "world"):
                            st.rerun()
                    else:
    ...
    with st.container(border=True):
            ...
            if not st.session_state.world_edit_mode:
                ...
                with c1:
                    if st.button("💾 저장", key="world_save_btn", use_container_width=True):
                        ...
                        if ok: st.toast("저장 완료", icon="✅")
                        else: st.toast(f"저장 실패: {msg}".strip(), icon="⚠️")
                        st.session_state.world_edit_mode = False
                        st.rerun()
                with c2:
        ...
        if not st.session_state.world_edit_mode:
            ...
            if saved: st.markdown(f"<div class='view-box'>{saved}</div>", unsafe_allow_html=True)
            else: st.markdown("<div class='view-box' style='color: rgba(0,0,0,0.45)'>설명을 입력하거나 파일을 업로드하세요.</div>", unsafe_allow_html=True)
        else:
            st.session_state.world_draft = st.text_area("세계관 내용", value=st.session_state.world_draft, height=220, label_visibility="collapsed")
    ...
    with st.container(border=False):
        ...
        with c1: st.markdown("<h2 class='section-title'>📌 플롯</h2>", unsafe_allow_html=True)
        with c2:
    ...
    for ep_no, item, raw_keys in items:
        ...
        with st.expander(f"#{ep_no}화", expanded=False):
            ...
            with top[0]:
                ...
                st.markdown(f"""<div class="episode-card"><div class="episode-header">{ep_no}화</div>{title_html}<div class="episode-summary">{summary}</div></div>""", unsafe_allow_html=True)
            with top[1]:
                if st.button("🗑 삭제", key=f"del_ep_{ep_no}", use_container_width=True):
                    if hist_path:
                        for k in raw_keys: history.pop(k, None)
